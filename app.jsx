  ...
  return (
    <div className="p-6 max-w-[1600px] mx-auto bg-slate-50 min-h-screen font-sans">
      <div className="flex justify-between items-center mb-6">
        ...
        <button onClick={resetDefaults} className="flex items-center gap-2 px-4 py-2 bg-white border border-slate-300 rounded-md hover:bg-slate-100 text-sm font-medium transition-colors shadow-sm">
          <RefreshCw size={16} /> Reset Assumptions
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-4 gap-6">
        ...
        <div className="xl:col-span-1 space-y-4 h-[calc(100vh-100px)] overflow-y-auto pr-2 pb-10">
            ...
            <div className="bg-white rounded-lg shadow border border-slate-200 overflow-hidden">
                <div className="px-4 py-3 bg-slate-100 border-b border-slate-200 font-bold text-sm text-slate-800 flex items-center gap-2">
                  <Sprout size={16}/> Phase Timing & Land
                </div>
                <div className="p-4 space-y-3 text-xs">
                    <div className="grid grid-cols-2 gap-3">
                        <div><label className="block mb-1 text-slate-500 font-medium">Phase 1 Year</label><input type="number" className="w-full p-2 border rounded" name="phase1Year" value={inputs.phase1Year} onChange={handleInputChange} /></div>
                        <div><label className="block mb-1 text-slate-500 font-medium">Phase 2 Year</label><input type="number" className="w-full p-2 border rounded" name="phase2Year" value={inputs.phase2Year} onChange={handleInputChange} /></div>
                        <div><label className="block mb-1 text-slate-500 font-medium">Phase 3 Year</label><input type="number" className="w-full p-2 border rounded" name="phase3Year" value={inputs.phase3Year} onChange={handleInputChange} /></div>
                        <div><label className="block mb-1 text-slate-500 font-medium">Ha Per Phase</label><input type="number" className="w-full p-2 border rounded" name="haPerPhase" value={inputs.haPerPhase} onChange={handleInputChange} /></div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                        <div><label className="block mb-1 text-slate-500 font-medium">Olive Split (0.5)</label><input type="number" step="0.1" className="w-full p-2 border rounded" name="oliveHaPercent" value={inputs.oliveHaPercent} onChange={handleInputChange} /></div>
                        <div><label className="block mb-1 text-slate-500 font-medium">Carob Split (0.5)</label><input type="number" step="0.1" className="w-full p-2 border rounded" name="carobHaPercent" value={inputs.carobHaPercent} onChange={handleInputChange} /></div>
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow border border-slate-200 overflow-hidden">
                <div className="px-4 py-3 bg-slate-100 border-b border-slate-200 font-bold text-sm text-slate-800 flex items-center gap-2">
                  <Factory size={16}/> CAPEX (Unit Costs TND)
                </div>
                <div className="p-4 space-y-3 text-xs">
                    <div><label className="block mb-1 text-slate-500 font-medium">Well Depth (m)</label><input type="number" className="w-full p-2 border rounded" name="wellDepth" value={inputs.wellDepth} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Drilling (TND/m)</label><input type="number" className="w-full p-2 border rounded" name="wellCostPerMeter" value={inputs.wellCostPerMeter} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Pump Cost</label><input type="number" className="w-full p-2 border rounded" name="pumpCost" value={inputs.pumpCost} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Irrigation (Per Ha)</label><input type="number" className="w-full p-2 border rounded" name="irrigationPerHa" value={inputs.irrigationPerHa} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Tree Price (Olive)</label><input type="number" className="w-full p-2 border rounded" name="treeOliveCost" value={inputs.treeOliveCost} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Tree Price (Carob)</label><input type="number" className="w-full p-2 border rounded" name="treeCarobCost" value={inputs.treeCarobCost} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Tractor (One-off)</label><input type="number" className="w-full p-2 border rounded" name="tractorCost" value={inputs.tractorCost} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Chemical Factory</label><input type="number" className="w-full p-2 border rounded" name="factoryCost" value={inputs.factoryCost} onChange={handleInputChange} /></div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow border border-slate-200 overflow-hidden">
                <div className="px-4 py-3 bg-slate-100 border-b border-slate-200 font-bold text-sm text-slate-800 flex items-center gap-2">
                  <Users size={16}/> OPEX (TND)
                </div>
                <div className="p-4 space-y-3 text-xs">
                    <div><label className="block mb-1 text-slate-500 font-medium">Eng. Salary (Mo)</label><input type="number" className="w-full p-2 border rounded" name="engineerSalary" value={inputs.engineerSalary} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Guard Salary (Mo)</label><input type="number" className="w-full p-2 border rounded" name="guardianSalary" value={inputs.guardianSalary} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Electricity (Well/Yr)</label><input type="number" className="w-full p-2 border rounded" name="electricityPerWell" value={inputs.electricityPerWell} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Fertilizer (Ha/Yr)</label><input type="number" className="w-full p-2 border rounded" name="fertilizerPerHa" value={inputs.fertilizerPerHa} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Harvest Olive (Kg)</label><input type="number" step="0.01" className="w-full p-2 border rounded" name="harvestLaborOlive" value={inputs.harvestLaborOlive} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Admin/Legal (Yr)</label><input type="number" className="w-full p-2 border rounded" name="adminLegalTND" value={inputs.adminLegalTND} onChange={handleInputChange} /></div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-lg shadow border border-slate-200 overflow-hidden">
                <div className="px-4 py-3 bg-slate-100 border-b border-slate-200 font-bold text-sm text-slate-800 flex items-center gap-2">
                  <DollarSign size={16}/> Revenue & Tax
                </div>
                <div className="p-4 space-y-3 text-xs">
                    <div><label className="block mb-1 text-slate-500 font-medium">Oil Bulk Price (TND)</label><input type="number" step="0.1" className="w-full p-2 border rounded" name="oliveOilPriceBulk" value={inputs.oliveOilPriceBulk} onChange={handleInputChange} /></div>
                    <div><label className="block mb-1 text-slate-500 font-medium">Gum Price (TND)</label><input type="number" className="w-full p-2 border rounded" name="carobGumPrice" value={inputs.carobGumPrice} onChange={handleInputChange} /></div>
                    <div className="grid grid-cols-2 gap-3">
                        <div><label className="block mb-1 text-slate-500 font-medium">Tax Ramzi</label><input type="number" step="0.01" className="w-full p-2 border rounded" name="taxRateRamzi" value={inputs.taxRateRamzi} onChange={handleInputChange} /></div>
                        <div><label className="block mb-1 text-slate-500 font-medium">Tax Mahdi</label><input type="number" step="0.01" className="w-full p-2 border rounded" name="taxRateMahdi" value={inputs.taxRateMahdi} onChange={handleInputChange} /></div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className="xl:col-span-3">
            ...
            <div className="flex border-b border-gray-200 mb-4">
              <button onClick={() => setActiveTab('corporate')} className={`px-4 py-2 text-sm font-medium flex items-center gap-2 ${activeTab === 'corporate' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
                <Factory size={16}/> 1. Corporate Horizon
              </button>
              <button onClick={() => setActiveTab('pocket')} className={`px-4 py-2 text-sm font-medium flex items-center gap-2 ${activeTab === 'pocket' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
                <Users size={16}/> 2. Pocket Cash
              </button>
              <button onClick={() => setActiveTab('summary')} className={`px-4 py-2 text-sm font-medium flex items-center gap-2 ${activeTab === 'summary' ? 'border-b-2 border-blue-500 text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}>
                <TrendingUp size={16}/> 3. Summary
              </button>
            </div>
                                ...
                                {calculations.yearlyData.map((row) => (
                                    <tr key={row.year}>
                                        ...
                                        <td className={tdClass}>
                                            {row.cashCall ? <span className="text-red-500 text-xs font-bold bg-red-100 px-2 py-1 rounded">CASH CALL</span> : <span className="text-green-600 text-xs font-bold bg-green-100 px-2 py-1 rounded">DIVIDEND</span>}
                                            {row.isFactory && <span className="ml-2 bg-purple-100 text-purple-800 text-xs font-bold px-2 py-1 rounded">FACTORY</span>}
                                        </td>
                                        <td className={`${tdClass} text-right text-gray-500`}>
                                            {Math.round(row.ramziPocket / (row.ramziPocket > 0 ? (1-inputs.taxRateRamzi) : (1+inputs.transferFriction))).toLocaleString()}
                                        </td>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-slate-900 text-white rounded-xl shadow-lg overflow-hidden">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div className="bg-slate-800 p-4 rounded-lg text-center mt-2">
                                ...
                                <p className="text-3xl font-bold text-white">€{Math.round(calculations.cumulativeCashRamzi + (calculations.totalAssetValue/2)).toLocaleString()}</p>
                    ...
                    <div className="bg-slate-900 text-white rounded-xl shadow-lg overflow-hidden">
                        ...
                        <div className="p-6 space-y-5">
                            ...
                            <div className="bg-slate-800 p-4 rounded-lg text-center mt-2">
                                <p className="text-xs text-slate-400 uppercase tracking-wider mb-1">Total Wealth Creation</p>
                                <p className="text-3xl font-bold text-white">€{Math.round(calculations.cumulativeCashMahdi + (calculations.totalAssetValue/2)).toLocaleString()}</p>
                                <p className="text-sm text-blue-400 font-bold mt-2 bg-blue-900/30 inline-block px-3 py-1 rounded-full">
